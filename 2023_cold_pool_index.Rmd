...
coords <- terra::crds(
  terra::rast(coldpool:::ebs_bottom_temperature), 
  na.rm = FALSE)

for(i in start_year:end_year) {
  sel_layer_df <- data.frame(x = coords[,1],
                             y = coords[,2],
                             temperature = terra::values(terra::rast(coldpool:::ebs_bottom_temperature))[,i])
...
coords <- terra::crds(
  terra::rast(coldpool:::nbs_ebs_bottom_temperature), 
  na.rm = FALSE)

for(i in 1:end_year) {
  sel_layer_df <- data.frame(x = coords[,1],
                             y = coords[,2],
                             temperature = terra::values(terra::rast(coldpool:::nbs_ebs_bottom_temperature))[,i])
  sel_layer_df <- sel_layer_df[!is.na(sel_layer_df$temperature),]
  sel_layer_df$year <- year_vec[i]
...
for(ii in 1:length(plot_years)) {
  ...
  sel_bt_layer_df <- data.frame(x = coords[,1],
                             y = coords[,2],
                             temperature = terra::values(sel_bt_raster)[,grep(pattern = plot_years[ii], x = names(sel_bt_raster))])
  ...
  sel_sst_layer_df <- data.frame(x = coords[,1],
                             y = coords[,2],
                             temperature = terra::values(sel_sst_raster)[,grep(pattern = plot_years[ii], x = names(sel_sst_raster))])
...
bt_df_sebs <- cbind(as.data.frame(terra::crds(bt_raster_sebs)),
      as.data.frame(bt_raster_sebs)) |>
  tidyr::pivot_longer(cols = 3:(dim(bt_raster_sebs)[3] + 2)) |>
  dplyr::rename(year = name,
                bt = value)

bt_raster_nbs <- terra::rast(coldpool::nbs_ebs_bottom_temperature)

bt_raster_2018 <- terra::mask(bt_raster_nbs[[3]],
...
bt_df_nbs <- cbind(as.data.frame(terra::crds(bt_raster_nbs)),
                    as.data.frame(bt_raster_nbs)) |>
  tidyr::pivot_longer(cols = 3:(dim(bt_raster_nbs)[3] + 2)) |>
  dplyr::rename(year = name,
                bt = value) |>
  dplyr::filter(year != 2018)

bt_df_nbs_2018 <- cbind(as.data.frame(terra::crds(bt_raster_2018)),
                   as.data.frame(bt_raster_2018)) |>
  tidyr::pivot_longer(cols = 3:(dim(bt_raster_2018)[3] + 2)) |>
  dplyr::rename(year = name,
                bt = value)
...
for(ii in 1:length(year_vec)) {
  ...
  if(year_vec[ii] %in% nbs_years) {
    sel_year <- dplyr::filter(bt_df_nbs, year == year_vec[ii])
  } else if (year_vec[ii] == 2018) {
    sel_year <- dplyr::filter(bt_df_nbs_2018, year == year_vec[ii])
  } else {
    sel_year <- dplyr::filter(bt_df_sebs, year == year_vec[ii])
  }
  ...
  temp_map <- ggplot() +
      ggplot2::geom_sf(data = nbs_layers$akland, 
                       fill = "grey70", 
                       color = "black") +
      geom_tile(data = sel_year,
                mapping = aes(x = x, 
                              y = y,
                              fill = cut(bt, breaks = temp_breaks))) +
