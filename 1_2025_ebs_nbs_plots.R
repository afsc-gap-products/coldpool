...
# Bottom and surface temperature path plot ---------------------------------------------------------

plot_btst_path <- 
  ggplot(
    data = cold_pool_index, 
    mapping = 
      aes(
        x = MEAN_GEAR_TEMPERATURE, 
        y = MEAN_SURFACE_TEMPERATURE, 
        label = substring(YEAR, nchar(YEAR)-1), 
        color = YEAR, 
        group = YEAR > 2020)
  ) +
  geom_vline(
  ...
  scale_x_continuous(name = expression('Bottom temperature ('*degree*'C)')) +
  scale_y_continuous(name = expression('Sea surface temperature ('*degree*'C)')) +
  facet_wrap(~"EBS shelf summer bottom trawl survey") +
...
bt_raster_2018 <- terra::mask(
  bt_raster_nbs[[names(bt_raster_nbs) == 2018]],
  dplyr::filter(nbs_layers$survey.grid,
                STATION %in% c("V-01", "U-01", "T-01", "S-01", "R-01",
...
st_clust <-
  cluster_spatraster(
    x = terra::unwrap(coldpool::ebs_surface_temperature),
    scale_vars = TRUE,
    dist_method = "euclidean",
    hclust_method = "complete"
  )
