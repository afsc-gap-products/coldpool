...
# Retrieve area-weighted surface temperature
channel <- coldpool:::get_connected()
...
temperature_df <- RODBC::sqlQuery(channel = channel,
                                  coldpool::sql_to_rqry(system.file("sql", 
...
```{r loocv}
  ...
  loocv_gear_temp(temp_data_path =  here::here("data", "2021-09-28_index_hauls_temperature_data.csv"),
...
cpa_df <- interpolation_wrapper(temp_data_path = here::here("data", "2021-09-28_index_hauls_temperature_data.csv"),
                                proj_crs = coldpool:::ebs_proj_crs,
                                cell_resolution = 5000,
                                select_years = 1982,year(Sys.Date()), 
...
area_vs_interp_bt_df <- area_weighted_standard_df %>% 
  dplyr::select(YEAR, AVGBSBT) %>%
  dplyr::rename(`BT Standard`  = AVGBSBT) %>%
  dplyr::full_join(area_weighted_plusnw_df %>%
                       dplyr::select(YEAR, AVGBSBT) %>%
...
area_vs_interp_sst_df <- area_weighted_standard_df %>% 
