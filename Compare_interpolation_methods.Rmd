...
```{r loocv}
  ...
  coldpool::loocv_gear_temp(temp_data_path =  here::here("data", "index_hauls_temperature_data.csv"),
                  proj_crs = proj_crs) # update file name to specify data file 
...
cpa_df <- interpolation_wrapper(temp_data_path = here::here("data", "index_hauls_temperature_data.csv"),
                                ...
                                select_years = c(1982:year(Sys.Date())), 
...
compare_temp_df <- readRDS(file = here::here("data", "ebs_bttemp_plus_nw_lt200m.rds")) %>%
  ...
  dplyr::rename(AREA_WEIGHTED_BT = AVGBSBT,
                AREA_WEIGHTED_SST = AVGBSST) %>%
  dplyr::full_join(coldpool:::cold_pool_index) |>
  dplyr::rename(INTERPOLATED_SST = MEAN_SURFACE_TEMPERATURE,
                INTERPOLATED_BT = MEAN_GEAR_TEMPERATURE)

ggplot(data = compare_temp_df %>%
         tidyr::pivot_longer(cols = c("AREA_WEIGHTED_BT", "AREA_WEIGHTED_SST", "INTERPOLATED_SST", "INTERPOLATED_BT")),
       aes(x = YEAR, 
           y = value,
           color = name,
           shape = name)) +
